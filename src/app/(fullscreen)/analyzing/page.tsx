  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (let i = STEP_PHASES.length - 1; i >= 0; i--) {
        if (elapsed >= STEP_PHASES[i].at) {
          setStepText(STEP_PHASES[i].text);
          break;
        }
      }
  ...
  return (
    ...
    >
      ...
      <motion.h2
        ...
        className="z-10 mb-6 text-center text-xl font-bold text-white md:text-2xl"
      >
      ...
      {previews.length > 0 && (
        <div className="relative z-10 mb-8 h-20 w-full max-w-md overflow-hidden">
          <motion.div
      ...
      {/* Circular progress */}
      <div className="relative z-10 mb-8" style={{ width: 180, height: 180 }}>
      ...
      <div className="z-10 flex min-h-[3rem] items-center justify-center">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="text-center text-lg font-medium md:text-xl"
          >
      ...
      {isComplete && (
        ...
        >
          <p className="animate-pulse text-sm text-white/60">結果ページに移動します...</p>
        </motion.div>
      )}
    </motion.div>
  );
